...
const getLeastTradeByDong = async (req, res) => {
    const query = 'SELECT name, gu, dong, jibun, size, MAX(year) AS year, MAX(month) AS month, MAX(tradePrice) AS tracePrice, MAX(created) AS created FROM tradeInfo WHERE TRIM(gu) = ? AND TRIM(dong) = ? GROUP BY name, gu, dong, jibun, size;';
...
const getLeastRentByDong = async (req, res) => {
    const query = 'SELECT name, gu, dong, jibun, size, MAX(year) AS year, MAX(month) AS month, MAX(rentPrice) AS rentPrice, MAX(created) AS created FROM rentInfo WHERE TRIM(gu) = ? AND TRIM(dong) = ? GROUP BY name, gu, dong, jibun, size;';

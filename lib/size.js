...
const getTradeSizeInfo = async (req, res) => {
    const query = 'SELECT DISTINCT size FROM jipsa.tradeInfo WHERE TRIM(gu) = ? AND TRIM(dong) = ? AND TRIM(jibun) = ? ORDER BY size ASC;';
    ...
    results.count = 0;
    results.gu = gu.trim();
    results.dong = dong.trim();
    results.jibun = jibun.trim();
    results.size = [];
...
const getRentSizeInfo = async (req, res) => {
    const query = 'SELECT DISTINCT size FROM jipsa.rentInfo WHERE TRIM(gu) = ? AND TRIM(dong) = ? AND TRIM(jibun) = ? ORDER BY size ASC;';

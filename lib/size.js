...
const getTradeSizeInfo = async (req, res) => {
    const query = 'SELECT DISTINCT size FROM jipsa.tradeInfo TRIM(gu) = ? AND TRIM(dong) = ? AND TRIM(jibun) = ?;';
...
const getRentSizeInfo = async (req, res) => {
    const query = 'SELECT DISTINCT size FROM jipsa.rentInfo TRIM(gu) = ? AND TRIM(dong) = ? AND TRIM(jibun) = ?;';
